(ns game-software.actor
  "GameOpsActor — the JSIC 3914 ゲームソフトウェア業 (game software
  production) actor as a `langgraph.graph/state-graph` (per ADR-2607011000 /
  CLAUDE.md Actors section). One graph run = one production operation
  ...
  The unconditional invariant: the GameOpsAdvisor can never directly commit
  a record the GameProductionGovernor refuses — every commit-record! call is
  gated behind `:decide`, and build commits carry the actual design report
  (`game-software.governor/build-design-report`, computed from
  kotoba-lang/game-production) so what was approved is exactly what was
  built."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [game-software.advisor :as advisor]
            [game-software.governor :as governor]
            [game-software.store :as store]))

(defn build-graph
  "Build a compiled GameOpsActor graph. `store` implements
  `game-software.store/Store`. `advisor` implements
  `game-software.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                  (fn [{:keys [request proposal verdict]}]
                    (let [record (cond-> {:title-id (:title-id request)
                                          :op (:op proposal)
                                          :payload proposal}
                                   (= :build (:op proposal))
                                   (assoc :design (or (:design verdict)
                                                      (governor/build-design-report
                                                       (:spec request)))))]
...
(defn approve!
  ...
  rating/licence/design decision)."
  [graph thread-id]
